...
function codeverify() {
    var email=document.getElementById('number-email').value;
    var number="+91"+document.getElementById('number-email').value;
...
function authenticate(){
    ...
    if(isValidPhoneNumber("+91"+number)){
        const xhr = new XMLHttpRequest();
            xhr.open("GET", "../assets/database/Host/check_number.php?number=" + number, true);
...
function doPost(){
    ...
    const address = document.getElementById("host_address").value;
    const number = document.getElementById("number-email").value;
    const usecase = document.getElementById("usecase").querySelector('input[name="usecase"]:checked').value;
    ...
    fetch("../assets/database/Host/Host_Signup.php", {
        method: "POST",
        headers: {
    ...
    .then(data => {
        ...
        window.location.href="Host Dashboard/Host_Dashboard.php";
    })
    .catch(error => {
...
function use(){
    ...
    document.getElementById('usecase-drones').style.display='block';
}